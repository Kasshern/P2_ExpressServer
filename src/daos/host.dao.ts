...
export async function getSocialEventsByHostUserId(id: number): Promise<SocialEvent[]> {
    const sql = 'SELECT events.* FROM users INNER JOIN events ON users.id = events.host_id \
                WHERE users.id = $1';
